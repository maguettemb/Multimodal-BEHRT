...
def str_to_date(d):
    ...
    raise TypeError(f"Unsupported date format or type: {type(d)} → {d}")
...
def delay(d1, d2, unit='day'):
    """
    Calculates the delay between two dates in days, weeks, or months.
    
    Parameters:
    - d1, d2: date strings or datetime objects
    - unit: 'day', 'week', or 'month'
    
    Returns:
    - Integer delay (positive or negative depending on order)
    """
    ...
    converters = {
        'day': lambda d: int(d),
        'week': lambda d: int(np.ceil(d / 7)),
        'month': lambda d: int(np.ceil(d / 30.4375))  # moyenne calendaire
    }

    if unit not in converters:
        raise ValueError(f"Invalid unit '{unit}'. Must be 'day', 'week' or 'month'.")

    return converters[unit](delta_days)
...
def cal_therapies_(df, window=14):
